...
def interactive_mode() -> int:
    ...
    try:
        print(
            f"{Fore.CYAN}=== Metadata Multitool Interactive Mode ==={Style.RESET_ALL}"
        )
        print(
            f"{Fore.YELLOW}Welcome! This mode will guide you through common workflows.{Style.RESET_ALL}\n"
        )
...
def show_main_menu() -> str:
    ...
    while True:
        choice = (
            input(f"\n{Fore.GREEN}Choose an option (1-5, q): {Style.RESET_ALL}")
            .strip()
            .lower()
        )
...
def handle_clean_workflow(config: Dict[str, Any]) -> None:
    ...
    while True:
        path_input = input(
            f"{Fore.GREEN}Enter path to images (file or directory): {Style.RESET_ALL}"
        ).strip()
        if not path_input:
    ...
    # Get output folder
    copy_folder = input(
        f"{Fore.GREEN}Output folder (default: safe_upload): {Style.RESET_ALL}"
    ).strip()
    if not copy_folder:
...
def handle_poison_workflow(config: Dict[str, Any]) -> None:
    ...
    while True:
        preset_choice = input(
            f"{Fore.GREEN}Choose preset (1-3): {Style.RESET_ALL}"
        ).strip()
        if preset_choice == "1":
            preset = "label_flip"
            break
    ...
    if preset == "label_flip":
        true_hint = input(
            f"{Fore.GREEN}Enter true content hint (optional): {Style.RESET_ALL}"
        ).strip()
    ...
    # Get rename pattern
    rename_pattern = input(
        f"{Fore.GREEN}Rename pattern (optional, e.g., '{{stem}}_toaster'): {Style.RESET_ALL}"
    ).strip()

    # Get CSV mapping
    csv_path = input(
        f"{Fore.GREEN}CSV mapping file (optional): {Style.RESET_ALL}"
    ).strip()
    mapping = {}
    if csv_path:
        try:
    ...
    try:
        ...
        if dry_run:
            print(
                f"{Fore.CYAN}DRY RUN: Would poison {total} images with preset '{preset}'...{Style.RESET_ALL}"
            )
            for i, img in enumerate(images, 1):
        ...
        print(
            f"{Fore.CYAN}Poisoning {total} images with preset '{preset}'...{Style.RESET_ALL}"
        )
        ...
        if total >= batch_size and max_workers > 1:
            ...
            def process_single_image(img_path: Path) -> tuple[bool, str]:
                try:
                    caption, tags = make_caption(
                        preset, true_hint or img_path.stem, mapping
                    )
                    ...
                    try:
                        write_metadata(
                            img_path, caption, tags, xmp=xmp, iptc=iptc, exif=exif
                        )
                    except Exception as e:
            ...
            print(
                f"{Fore.YELLOW}Poisoned labels for {successful}/{total_processed} image(s) with preset '{preset}'.{Style.RESET_ALL}"
            )
        else:
            # Sequential processing
            count = 0
            for i, img in enumerate(images, 1):
                try:
                    ...
                    try:
                        write_metadata(
                            img, caption, tags, xmp=xmp, iptc=iptc, exif=exif
                        )
                    except Exception as e:
                        if verbose:
                            print(
            ...
            write_log(base, log)
            print(
                f"{Fore.YELLOW}Poisoned labels for {count}/{total} image(s) with preset '{preset}'.{Style.RESET_ALL}"
            )
...
def handle_revert_workflow() -> None:
    ...
    while True:
        path_input = input(
            f"{Fore.GREEN}Enter path to directory: {Style.RESET_ALL}"
        ).strip()
        if not path_input:
            print(f"{Fore.RED}Path cannot be empty.{Style.RESET_ALL}")
            continue
    ...
    while True:
        ...
        if choice == "1":
            ...
            try:
                new_batch_size = int(
                    input(
                        f"{Fore.GREEN}Enter new batch size (current: {get_config_value(config, 'batch_size', 100)}): {Style.RESET_ALL}"
                    )
                )
                if new_batch_size > 0:
                    config["batch_size"] = new_batch_size
                    print(
                        f"{Fore.GREEN}Batch size updated to {new_batch_size}{Style.RESET_ALL}"
                    )
                else:
            ...
            try:
                new_max_workers = int(
                    input(
                        f"{Fore.GREEN}Enter new max workers (current: {get_config_value(config, 'max_workers', 4)}): {Style.RESET_ALL}"
                    )
                )
                if new_max_workers > 0:
                    config["max_workers"] = new_max_workers
                    print(
                        f"{Fore.GREEN}Max workers updated to {new_max_workers}{Style.RESET_ALL}"
                    )
                else:
            ...
            try:
                ...
                print(
                    f"{Fore.GREEN}Configuration saved to {config_path}{Style.RESET_ALL}"
                )
            except Exception as e:
...
def handle_batch_workflow(config: Dict[str, Any]) -> None:
    ...
    while True:
        path_input = input(
            f"{Fore.GREEN}Enter path to images directory: {Style.RESET_ALL}"
        ).strip()
        if not path_input:
            print(f"{Fore.RED}Path cannot be empty.{Style.RESET_ALL}")
            continue
    ...
    if get_yes_no("Change batch settings? (y/n): ", default=False):
        try:
            new_batch_size = int(
                input(
                    f"{Fore.GREEN}Enter batch size (current: {batch_size}): {Style.RESET_ALL}"
                )
            )
            if new_batch_size > 0:
        ...
        try:
            new_max_workers = int(
                input(
                    f"{Fore.GREEN}Enter max workers (current: {max_workers}): {Style.RESET_ALL}"
                )
            )
            if new_max_workers > 0:
    ...
    if choice == "1":
        ...
        print(
            f"{Fore.YELLOW}Poison workflow would be implemented here.{Style.RESET_ALL}"
        )
        print(
            f"{Fore.YELLOW}For now, use the main poison workflow or command line.{Style.RESET_ALL}"
        )
