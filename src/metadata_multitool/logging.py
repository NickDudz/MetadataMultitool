...
class OperationLogger:
    ...
    def log_operation_end(
        self, operation: str, success: bool, details: Dict[str, Any]
    ) -> None:
        """
    ...
    def log_batch_progress(
        self, operation: str, processed: int, total: int, errors: int
    ) -> None:
        """
        ...
        self.logger.info(
            f"Batch {operation} progress: {processed}/{total} ({progress:.1f}%) - {errors} errors"
        )
...
def get_logger(
    log_file: Optional[Path] = None, log_level: str = "INFO"
) -> OperationLogger:
    """
...
def log_operation_summary(
    logger: OperationLogger, operation: str, results: Dict[str, Any]
) -> None:
    """
