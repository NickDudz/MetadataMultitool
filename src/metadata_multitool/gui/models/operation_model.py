...
class OperationModel:
    ...
    def update_progress(
        self, processed: int, current_file: Optional[str] = None
    ) -> None:
    ...
    def complete_operation(self, success: bool = True) -> None:
        ...
        with self._lock:
            if success:
                ...
                self.progress = 1.0
                self.status_message = (
                    f"Completed successfully. {self.successful_files} files processed."
                )
            else:
                self.status = OperationStatus.ERROR
                self.status_message = (
                    f"Operation failed. {self.failed_files} files failed."
                )
