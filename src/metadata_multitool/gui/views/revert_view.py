...
class RevertView:
    ...
    def _setup_action_buttons(self) -> None:
        ...
        revert_btn = ttk.Button(
            buttons_frame, text="Revert", command=self._revert_files
        )
        ...
        refresh_btn = ttk.Button(
            buttons_frame, text="Refresh", command=self._refresh_files
        )
    ...
    def _dry_run(self) -> None:
        ...
        result_text = (
            f"DRY RUN: Would revert {len(files_to_revert)} files\n\n"
            + "\n".join(results)
        )
    ...
    def _start_revert_operation(self, directory: Path) -> None:
        ...
        files_count = self.files_listbox.size()
        show_info(
            "Revert Operation", f"Would revert {files_count} files in {directory}"
        )
