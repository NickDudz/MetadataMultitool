...
from ..utils.validation_utils import validate_path, validate_batch_size, validate_max_workers
...
class CleanView:
    ...
    def setup(self, parent: tk.Widget) -> None:
        ...
        # Output folder
        ttk.Label(self.frame, text="Output Folder:").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        browse_btn = ttk.Button(folder_frame, text="Browse...", command=self._browse_output_folder)
    ...
    def _setup_file_filters(self) -> None:
        ...
        filters_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(10, 0))
    ...
    def _setup_action_buttons(self) -> None:
        ...
        buttons_frame.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(20, 0))
    ...
    def _start_clean_operation(self, files) -> None:
        ...
        show_info("Clean Operation", f"Would clean {len(files)} files to {self.output_folder_var.get()}")
    ...
    def _show_dry_run_results(self, files) -> None:
        ...
        for file_path in files:
            ...
            results.append(f"{file_path.name} ({size_str}) → {output_folder}/{file_path.name}")
