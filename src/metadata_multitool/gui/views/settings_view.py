...
class SettingsView:
    ...
    def _setup_general_settings(self, parent: tk.Widget) -> None:
        ...
        # Log level
        ttk.Label(parent, text="Log Level:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10), pady=(0, 5))
        self.log_level_var = tk.StringVar(value=self.config_model.get_value("log_level", "INFO"))
        ...
        self.backup_var = tk.BooleanVar(value=self.config_model.get_value("backup_before_operations", True))
        ...
        # Verbose mode
        self.verbose_var = tk.BooleanVar(value=self.config_model.get_value("verbose", False))
        ...
        # Quiet mode
        self.quiet_var = tk.BooleanVar(value=self.config_model.get_value("quiet", False))
        quiet_check = ttk.Checkbutton(
            parent, 
            text="Quiet output (minimal messages)", 
    ...
    def _setup_processing_settings(self, parent: tk.Widget) -> None:
        ...
        # Batch size
        ttk.Label(parent, text="Batch Size:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10), pady=(0, 5))
        ...
        ttk.Label(parent, text="Number of files to process in each batch").grid(row=0, column=2, sticky=tk.W, padx=(10, 0), pady=(0, 5))
        
        # Max workers
        ttk.Label(parent, text="Max Workers:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 5))
        self.max_workers_var = tk.StringVar(value=str(self.config_model.get_value("max_workers", 4)))
        max_workers_entry = ttk.Entry(parent, textvariable=self.max_workers_var, width=15)
        max_workers_entry.grid(row=1, column=1, sticky=tk.W, pady=(10, 5))
        ttk.Label(parent, text="Maximum number of parallel worker processes").grid(row=1, column=2, sticky=tk.W, padx=(10, 0), pady=(10, 5))
        
        # Progress bar
        self.progress_bar_var = tk.BooleanVar(value=self.config_model.get_value("progress_bar", True))
        ...
        # Supported formats
        ttk.Label(parent, text="Supported Formats:").grid(row=3, column=0, sticky=(tk.W, tk.N), padx=(0, 10), pady=(10, 5))
        formats_frame = ttk.Frame(parent)
        formats_frame.grid(row=3, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=(10, 5))
    ...
    def _setup_gui_settings(self, parent: tk.Widget) -> None:
        ...
        # Theme
        ttk.Label(parent, text="Theme:").grid(row=0, column=0, sticky=tk.W, padx=(0, 10), pady=(0, 5))
        self.theme_var = tk.StringVar(value=self.config_model.get_gui_setting("theme", "light"))
        ...
        # Window size
        ttk.Label(parent, text="Window Size:").grid(row=1, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 5))
        ...
        window_size_entry = ttk.Entry(parent, textvariable=self.window_size_var, width=15)
        window_size_entry.grid(row=1, column=1, sticky=tk.W, pady=(10, 5))
        ttk.Label(parent, text="Width x Height (e.g., 1000x700)").grid(row=1, column=2, sticky=tk.W, padx=(10, 0), pady=(10, 5))
        
        # Show thumbnails
        self.show_thumbnails_var = tk.BooleanVar(value=self.config_model.get_gui_setting("show_thumbnails", True))
        ...
        # Thumbnail size
        ttk.Label(parent, text="Thumbnail Size:").grid(row=3, column=0, sticky=tk.W, padx=(0, 10), pady=(10, 5))
        ...
        thumbnail_size_entry = ttk.Entry(parent, textvariable=self.thumbnail_size_var, width=15)
        thumbnail_size_entry.grid(row=3, column=1, sticky=tk.W, pady=(10, 5))
        ttk.Label(parent, text="Size in pixels (e.g., 64)").grid(row=3, column=2, sticky=tk.W, padx=(10, 0), pady=(10, 5))
        
        # Auto backup
        self.auto_backup_var = tk.BooleanVar(value=self.config_model.get_gui_setting("auto_backup", True))
        ...
        self.remember_folder_var = tk.BooleanVar(value=self.config_model.get_gui_setting("remember_last_folder", True))
        remember_folder_check = ttk.Checkbutton(
            parent, 
            text="Remember last used folder", 
            variable=self.remember_folder_var
        )
        remember_folder_check.grid(row=5, column=0, columnspan=3, sticky=tk.W, pady=(0, 5))
    ...
    def _setup_action_buttons(self) -> None:
        ...
        reset_btn = ttk.Button(buttons_frame, text="Reset to Defaults", command=self._reset_settings)
        reset_btn.grid(row=0, column=1, padx=(0, 10))
        
        cancel_btn = ttk.Button(buttons_frame, text="Cancel", command=self._cancel_settings)
    ...
    def _validate_inputs(self) -> bool:
        ...
        try:
            ...
            if 'x' in size_str:
                ...
                show_error("Invalid Input", "Window size format: WIDTHxHEIGHT (e.g., 1000x700)")
                return False
        except ValueError:
    ...
    def _update_config(self) -> None:
        ...
        self.config_model.set_gui_setting("show_thumbnails", self.show_thumbnails_var.get())
        self.config_model.set_gui_setting("thumbnail_size", int(self.thumbnail_size_var.get()))
        self.config_model.set_gui_setting("auto_backup", self.auto_backup_var.get())
        self.config_model.set_gui_setting("remember_last_folder", self.remember_folder_var.get())
