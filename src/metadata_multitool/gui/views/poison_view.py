...
class PoisonView:
    ...
    def _setup_preset_selection(self) -> None:
        ...
        preset_frame = ttk.LabelFrame(self.frame, text="Poison Preset Selection", padding="10")
        preset_frame.grid(row=0, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        preset_frame.columnconfigure(1, weight=1)
        
        # Preset selection label
        ttk.Label(preset_frame, text="Poison Preset:", font=("TkDefaultFont", 10, "bold")).grid(row=0, column=0, sticky=tk.W, pady=(0, 10))
        ...
        # Description label
        self.description_var = tk.StringVar(value="Replace labels with misleading ones to confuse AI training")
        description_label = ttk.Label(preset_frame, textvariable=self.description_var, wraplength=400)
    ...
    def _setup_true_hint(self) -> None:
        ...
        hint_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        hint_frame.columnconfigure(1, weight=1)
        
        ttk.Label(hint_frame, text="True Hint:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(hint_frame, text="(Optional hint about real content)").grid(row=0, column=2, sticky=tk.W, padx=(5, 0))
    ...
    def _setup_output_formats(self) -> None:
        ...
        formats_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        ...
        iptc_check = ttk.Checkbutton(metadata_frame, text="IPTC", variable=self.iptc_var)
        iptc_check.pack(side=tk.LEFT, padx=(0, 10))
        
        self.exif_var = tk.BooleanVar(value=False)
        exif_check = ttk.Checkbutton(metadata_frame, text="EXIF", variable=self.exif_var)
        ...
        sidecar_check = ttk.Checkbutton(sidecar_frame, text=".txt", variable=self.sidecar_var)
        sidecar_check.pack(side=tk.LEFT, padx=(0, 10))
        
        self.json_var = tk.BooleanVar(value=False)
        json_check = ttk.Checkbutton(sidecar_frame, text=".json", variable=self.json_var)
        json_check.pack(side=tk.LEFT, padx=(0, 10))
        
        self.html_var = tk.BooleanVar(value=False)
        html_check = ttk.Checkbutton(sidecar_frame, text=".html", variable=self.html_var)
    ...
    def _setup_rename_pattern(self) -> None:
        ...
        rename_frame.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        rename_frame.columnconfigure(1, weight=1)
        
        ttk.Label(rename_frame, text="Rename Pattern:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(rename_frame, text="(use {stem}, {rand})").grid(row=0, column=2, sticky=tk.W, padx=(5, 0))
    ...
    def _setup_csv_mapping(self) -> None:
        ...
        ttk.Label(csv_frame, text="CSV Mapping:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        browse_csv_btn = ttk.Button(csv_frame, text="Browse...", command=self._browse_csv_file)
    ...
    def _setup_file_filters(self) -> None:
        ...
        filters_frame.grid(row=5, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(0, 10))
        filters_frame.columnconfigure(1, weight=1)
        
        # Size filter
        ttk.Label(filters_frame, text="Size:").grid(row=0, column=0, sticky=tk.W, padx=(0, 5))
        ...
        ttk.Label(filters_frame, text="(e.g., 1MB, 500KB-2MB, >1GB)").grid(row=0, column=2, sticky=tk.W)
        
        # Date filter
        ttk.Label(filters_frame, text="Date:").grid(row=1, column=0, sticky=tk.W, padx=(0, 5), pady=(5, 0))
        ...
        ttk.Label(filters_frame, text="(e.g., 2024-01-01, 2024-01-01:2024-12-31)").grid(row=1, column=2, sticky=tk.W, pady=(5, 0))
        
        # Format filter
        ttk.Label(filters_frame, text="Formats:").grid(row=2, column=0, sticky=tk.W, padx=(0, 5), pady=(5, 0))
        ...
        ttk.Label(filters_frame, text="(space-separated)").grid(row=2, column=2, sticky=tk.W, pady=(5, 0))
        
        # Metadata filter
        self.metadata_var = tk.StringVar(value="all")
        metadata_frame = ttk.Frame(filters_frame)
        metadata_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(5, 0))
        
        ttk.Radiobutton(metadata_frame, text="All files", variable=self.metadata_var, value="all").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Radiobutton(metadata_frame, text="Has metadata", variable=self.metadata_var, value="has").pack(side=tk.LEFT, padx=(0, 10))
        ttk.Radiobutton(metadata_frame, text="No metadata", variable=self.metadata_var, value="none").pack(side=tk.LEFT)
        
    def _setup_processing_options(self) -> None:
        """Setup processing options (reuse from clean view)."""
        options_frame = ttk.LabelFrame(self.frame, text="Processing Options", padding="5")
        ...
        # Batch size
        ttk.Label(options_frame, text="Batch size:").grid(row=2, column=0, sticky=tk.W, padx=(0, 5))
        self.batch_size_var = tk.StringVar(value=str(self.config_model.get_value("batch_size", 100)))
        batch_size_entry = ttk.Entry(options_frame, textvariable=self.batch_size_var, width=10)
        batch_size_entry.grid(row=2, column=1, sticky=tk.W, padx=(0, 20))
        
        # Max workers
        ttk.Label(options_frame, text="Max workers:").grid(row=2, column=2, sticky=tk.W, padx=(0, 5))
        self.max_workers_var = tk.StringVar(value=str(self.config_model.get_value("max_workers", 4)))
        max_workers_entry = ttk.Entry(options_frame, textvariable=self.max_workers_var, width=10)
        max_workers_entry.grid(row=2, column=3, sticky=tk.W)
        
    def _setup_action_buttons(self) -> None:
        """Setup action buttons."""
        buttons_frame = ttk.Frame(self.frame)
        buttons_frame.grid(row=7, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(20, 0))
        
        process_btn = ttk.Button(buttons_frame, text="Process", command=self._process_files)
    ...
    def _browse_csv_file(self) -> None:
        ...
        file_path = filedialog.askopenfilename(title="Select CSV Mapping File", filetypes=filetypes)
        if file_path:
    ...
    def _start_poison_operation(self, files) -> None:
        ...
        show_info("Poison Operation", f"Would poison {len(files)} files with preset '{preset}'")
    ...
    def _show_dry_run_results(self, files) -> None:
        ...
        for file_path in files:
            ...
            size_str = f"{info.get('size', 0) / 1024 / 1024:.1f} MB" if info else "Unknown"
            results.append(f"{file_path.name} ({size_str}) → '{preset}' preset")
            
        # Show results in a dialog
        result_text = f"DRY RUN: Would poison {len(files)} files with preset '{preset}'\n\n" + "\n".join(results)
