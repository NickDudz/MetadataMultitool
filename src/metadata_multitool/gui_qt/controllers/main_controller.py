...
from ..services.cli_service import CLIService, CleanOptions, PoisonOptions, RevertOptions
...
class MainController(QObject):
    ...
    def execute_clean(self, options_dict: Dict[str, Any]) -> None:
        ...
        if validation_result["invalid"]:
            ...
            self.operation_error.emit(f"Invalid file formats: {', '.join(invalid_files)}")
            return
            
        # Create options from config and user input
        options = CleanOptions(
            output_folder=options_dict.get("output_folder", self.config_model.get_operation_default("clean", "output_folder", "safe_upload")),
            preserve_structure=options_dict.get("preserve_structure", self.config_model.get_operation_default("clean", "preserve_structure", True)),
    ...
    def execute_poison(self, options_dict: Dict[str, Any]) -> None:
        ...
        # Create options from config and user input
        default_formats = self.config_model.get_operation_default("poison", "output_formats", {
            "xmp": True, "iptc": True, "exif": False, "sidecar": True, "json": True, "html": False
        })
        
        options = PoisonOptions(
            preset=options_dict.get("preset", self.config_model.get_operation_default("poison", "preset", "label_flip")),
