...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.main_controller = MainController(
            self.file_model, self.config_model, self.operation_model, self.cli_service
        )
    ...
    def _setup_menus(self) -> None:
        ...
        add_files_action = QAction(
            self.icon_manager.get_icon("add"), "&Add Files...", self
        )
        ...
        add_folder_action = QAction(
            self.icon_manager.get_icon("folder"), "Add &Folder...", self
        )
        ...
        # Clear files action
        clear_files_action = QAction(
            self.icon_manager.get_icon("clear"), "&Clear All", self
        )
        ...
        settings_action = QAction(
            self.icon_manager.get_icon("settings"), "&Settings...", self
        )
    ...
    def _setup_connections(self) -> None:
        ...
        if self.main_view:
            ...
            self.main_view.tab_widget.currentChanged.connect(
                self._on_main_view_tab_changed
            )
    ...
    def _show_settings(self) -> None:
        ...
        if self.settings_dialog is None:
            self.settings_dialog = SettingsDialog(
                self.config_model, self.theme_manager, self
            )
...
<p><a href="https://github.com/NickDudz/MetadataMultitool">GitHub Repository</a></p>
    ...
    def _on_operation_progress(
        ...
        if current_file:
            self._update_status(
                f"Processing {current}/{total}: {Path(current_file).name}"
            )
        else:
